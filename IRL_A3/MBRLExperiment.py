...
def run_repetitions(n_timesteps, n_repetitions, eval_interval, gamma, learning_rate,
                    epsilon, n_planning_updates, wind_proportion):
    all_curves = []
    eval_timesteps = np.arange(0, n_timesteps, eval_interval)

    for rep in tqdm(range(n_repetitions), desc=f'n_plan={n_planning_updates}, wind={wind_proportion}'):
...
def experiment():
    ...
    for wind_proportion in wind_proportions:
        ...
        ts, curve = run_repetitions(n_timesteps, n_repetitions, eval_interval, gamma,
                                    learning_rate, epsilon, n_planning_updates=0,
        ...
        for n_plan in n_planning_updatess:
            ts, curve = run_repetitions(n_timesteps, n_repetitions, eval_interval, gamma,
                                        learning_rate, epsilon, n_planning_updates=n_plan,
                                        wind_proportion=wind_proportion)
